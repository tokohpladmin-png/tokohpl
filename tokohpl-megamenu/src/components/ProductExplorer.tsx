...
export function ProductExplorer({ products, filterOptions }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    return result;
  }, [products, query, brand, collection, size]);
    ...
    <div>
      ...
      <div className="border border-hpl-line bg-white mb-8">
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-hpl-line">
          ...
          {hasFilters && (
            <button onClick={clearAll} className="text-[11px] font-medium tracking-[0.12em] uppercase text-hpl-accent hover:text-hpl-ink transition-colors">
              Hapus Filter
            </button>
          )}
        </div>
