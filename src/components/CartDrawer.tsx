      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-hpl-line bg-hpl-ink">
          <div>
            ...
            <h2 className="text-[20px] font-light tracking-[-0.03em] text-white">
              {t('heading')}
              {qty > 0 && <span className="text-[14px] font-normal text-hpl-400 ml-2">{t('sheetsCount', { count: qty })}</span>}
            </h2>
          </div>
          <button type="button" onClick={closeDrawer} aria-label={t('closeAria')}
        ...
        {qty > 0 && (tier.rate > 0 || appliedCoupon) && (
          <div className="bg-hpl-accent px-6 py-2 text-[11px] font-semibold text-white flex items-center justify-between">
            <span>
              {tier.rate > 0 && `${t('discount')} ${Math.round(tier.rate * 100)}%`}
              {tier.rate > 0 && appliedCoupon && ' + '}
              {appliedCoupon && t('couponPrefix', { code: appliedCoupon.code })}
            </span>
            <span>{t('youSave', { amount: formatIDR(disc) ?? '' })}</span>
          </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full px-8 text-center">
              ...
              <p className="text-[13px] font-medium text-hpl-ink mb-1">{t('emptyTitle')}</p>
              <p className="text-[12px] text-hpl-500 mb-6">{t('emptyBody')}</p>
              <button onClick={closeDrawer} className="btn-ghost text-[10px] py-2.5 px-5">{t('viewProducts')}</button>
            </div>
          ) : (
                      ...
                      <div className="flex items-center justify-between gap-2 flex-wrap">
                        ...
                        <div className="text-right">
                          {(() => {
                            ...
                            return discounted ? (
                              <>
                                <p className="text-[11px] line-through text-hpl-400">{formatIDR(full) ?? tCommon('priceTBD')}</p>
                                <p className="text-[13px] font-semibold text-hpl-accent">{formatIDR(discounted) ?? tCommon('priceTBD')}</p>
                              </>
                            ) : (
                              <p className="text-[13px] font-semibold text-hpl-ink">{formatIDR(full) ?? tCommon('priceTBD')}</p>
                            );
                          })()}
                        </div>
                      </div>
          ...
          <div className="border-t border-hpl-line px-6 py-5 space-y-3 bg-white">
            ...
            <div className="space-y-2 pt-1">
              <div className="flex justify-between text-[12px]">
                ...
                <span className="text-hpl-ink">{formatIDR(sub) ?? tCommon('priceTBD')}</span>
              </div>
              {disc > 0 && (
                <div className="flex justify-between text-[12px]">
              ...
              {province ? (
                ...
                <div className="flex justify-between text-[12px]">
                  <span className="text-hpl-600">{t('shipping')}</span>
                  <span className="text-hpl-400 italic">{t('shippingCalculatedAtCheckout')}</span>
                </div>
              )}
              <div className="flex justify-between pt-2 border-t border-hpl-line">
                <span className="text-[12px] font-semibold text-hpl-ink">{t('total')}</span>
                <span className="text-[16px] font-semibold text-hpl-ink">{formatIDR(province ? order : total) ?? tCommon('priceTBD')}</span>
              </div>
            </div>
