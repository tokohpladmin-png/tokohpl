...
export function parseProductsFromCsv(csvText: string): Product[] {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    // Weight per sheet in kg; default 3.5 if absent or invalid
    const rawWeight = row['weight'] || row['berat'] || '';
    const parsedWeight = parseFloat(rawWeight.replace(/[^0-9.]/g, ''));
    const weight = isNaN(parsedWeight) || parsedWeight <= 0 ? 3.5 : parsedWeight;
