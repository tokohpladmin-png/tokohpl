            ...
            <div className="border border-hpl-line">
              ...
              <ul className="divide-y divide-hpl-line">
                {order.items.map((item) => {
                  const key = item.edgeBand
                    ? `${item.product.code}-eb-${item.edgeBand.size}`
                    : item.product.code;
                  return (
                    <li key={key} className="flex items-center justify-between gap-4 px-6 py-4">
                      <div className="min-w-0 flex-1">
                        <p className="text-[10px] tracking-[0.14em] uppercase text-hpl-500">
                          {item.edgeBand
                            ? `Edge Band · ${item.product.edgeBand?.code ?? ''}`
                            : `${item.product.brand} · ${item.product.code}`}
                        </p>
                        <p className="text-[13px] font-medium text-hpl-ink">
                          {item.edgeBand ? item.edgeBand.size : item.product.name}
                        </p>
                        <p className="text-[11px] text-hpl-500 mt-0.5">
                          {item.edgeBand
                            ? `${item.edgeBand.meters} m × ${formatIDR(item.edgeBand.pricePerMeter)} / m`
                            : `${formatIDR(item.product.price)} × ${item.quantity} lembar`}
                        </p>
                      </div>
                      <span className="text-[14px] font-semibold text-hpl-ink shrink-0">
                        {item.edgeBand
                          ? formatIDR(item.edgeBand.pricePerMeter * item.edgeBand.meters)
