          ...
          <div className="flex h-[68px] items-center justify-between gap-6">
            ...
            <Link href="/" className="shrink-0 flex items-center gap-2" aria-label="TokoHPL">
              <div className="flex items-center gap-2">
                ...
                <span className="display text-hpl-ink text-xl hidden sm:block">TokoHPL</span>
              </div>
            </Link>
            ...
            <nav className="hidden lg:flex items-center gap-8 text-[11px] font-semibold tracking-[0.16em] uppercase text-hpl-700">
              ...
              <div className="group relative py-6">
                ...
                <div className="invisible absolute left-1/2 top-full -translate-x-1/2 opacity-0 transition-all duration-200 group-hover:visible group-hover:opacity-100 z-[100]" style={{minWidth:'480px'}}>
                  <div className="mt-2 border border-hpl-line bg-white shadow-luxury py-4 px-2">
                    <div className="grid grid-cols-2 gap-x-6 px-3">
                      {/* EDL column */}
                      <div>
                        <Link href="/products?brand=EDL"
                          className="block text-[10px] font-bold tracking-[0.12em] uppercase text-hpl-ink mb-2 pb-2 border-b border-hpl-line hover:text-hpl-accent transition-colors">
                          EDL HPL
                        </Link>
                        {[
                          { label: 'Wood',    href: '/products?brand=EDL&collection=Wood' },
                          { label: 'Solid',   href: '/products?brand=EDL&collection=Solid' },
                          { label: 'Pattern', href: '/products?brand=EDL&collection=Pattern' },
                          { label: 'Marble',  href: '/products?brand=EDL&collection=Marble' },
                          { label: 'Stone',   href: '/products?brand=EDL&collection=Stone' },
                          { label: 'Metal',   href: '/products?brand=EDL&collection=Metal' },
                          { label: 'Aptico',  href: '/products?brand=EDL&collection=Aptico' },
                        ].map((item) => (
                          <Link key={item.href} href={item.href}
                            className="block py-1.5 text-[11px] font-medium tracking-[0.08em] text-hpl-600 hover:text-hpl-ink hover:translate-x-0.5 transition-all">
                            {item.label}
                          </Link>
                        ))}
                      </div>
                      ...
                      <div>
                        ...
                        {[
                          { label: 'Woods',    href: '/products?brand=LAMITAK&collection=Woods' },
                          { label: 'Solids',   href: '/products?brand=LAMITAK&collection=Solids' },
                          { label: 'Patterns', href: '/products?brand=LAMITAK&collection=Patterns' },
                        ].map((item) => (
                          <Link key={item.href} href={item.href}
                            className="block py-1.5 text-[11px] font-medium tracking-[0.08em] text-hpl-600 hover:text-hpl-ink hover:translate-x-0.5 transition-all">
                            {item.label}
                    ...
                    <Link href="/products"
                      className="block px-3 text-[11px] font-semibold tracking-[0.1em] uppercase text-hpl-ink hover:text-hpl-accent transition-colors">
                      Semua Produk →
                    </Link>
                  </div>
                </div>
